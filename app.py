...
from mpg_data import (get_data, insight_creator, last_10_creator, lin_reg,
                      money_format)
...
df_insights["Avg gallon cost"] = df_insights["Avg gallon cost"].apply(
    lambda x: "$" + str(x) + "0" if len(str(x)) < 4 else "$" + str(x)
)
df_insights["Cost to go one mile"] = "$" + (
    round(df_insights["Cost to go one mile (in cents)"] / 100, 2)
).astype(str).apply(lambda x: str(x) + "0" if len(str(x)) < 4 else str(x))
...
scatter1_options = [
    {"label": str(i), "value": i} for i in ["Miles Per Gallon", "Gallon Cost"]
]
...
scatter2_options = [
    {"label": str(i), "value": i}
    for i in ["Miles Per Gallon", "Cost To Drive One Mile"]
]

app.layout = html.Div(
    [
        dbc.Row(html.H1(children="MPG Dashboard", style={"textAlign": "center"})),
        ...
        html.Div(
            [
                ...
                dcc.RadioItems(
                    id="scatter1-radio", options=scatter1_radio_options, value="mov_avg"
                ),
            ],
            style={
                "width": "50%",
        ...
        dbc.Row(html.H2(children="Gas Insights", style={"textAlign": "center"})),
        html.Center(
            [
                html.Div(
                    [
        ...
        dbc.Row(html.H2(children="Last 10 Fillups", style={"textAlign": "center"})),
        html.Center(
            [
                html.Div(
                    [
...
@app.callback(
    ...
    [Input("scatter1-kind", "value"), Input("scatter1-radio", "value")],
)
def update_scatter1(graph_option, radio_option):
    if graph_option.startswith("Miles Per Gallon"):
        ...
        ytick = [
            i for i in range(math.floor(df["mpg"].min()), math.ceil(df["mpg"].max()))
        ]
        ...
        if radio_option == "values":
            y = df["gal_cost"]
            htemp = (
                df["gal_cost"].apply(money_format)
                + " on "
            ...
            htemp = (
                "$"
                + round(df["gal_cost"].rolling(window=5).mean(), 2).apply(
            ...
            line_color = "#5C7DAA"
        Y_t = y * 10
        yrange = [df["gal_cost"].min() - 0.2, df["gal_cost"].max() + 0.2]
        ytick = [
            i / 10 for i in range(math.floor(Y_t.min()), math.ceil(Y_t.max()) + 2, 2)
        ]
        tick_text = [
            money_format(i)
            for i in [
                i / 10
                for i in range(math.floor(Y_t.min()), math.ceil(Y_t.max()) + 2, 2)
            ]
        ]
    ...
    return {
        ...
        "layout": go.Layout(
            ...
            yaxis=dict(
                title=graph_option, range=yrange, tickvals=ytick, ticktext=tick_text
            ),
        ),
    }


@app.callback(Output("scatter2", "figure"), [Input("scatter2-kind", "value")])
def update_scatter2(graph_option):
    ...
    if graph_option.startswith("Miles"):
        ...
        htemp = (
            "<b>Miles Driven: </b>"
            + x.astype(str)
            + "<br><b>Miles Per Gallon: </b>"
            + y.astype(str)
            + "<br>"
            + df["date"].dt.strftime("%b %-d, %Y")
            + "<extra></extra>"
        )
        yrange = [y.min() - 0.5, y.max() + 0.5]
        ytick = [i for i in range(math.floor(y.min()), math.ceil(y.max()))]
        tick_text = [i if i % 2 == 0 else "" for i in ytick]
        annotation = (
            "On average, I got "
            + (round(df_i["MPG"].iloc[-1], 2)).astype(str)
            + " miles per gallon"
        )
        ...
        htemp = (
            "<b>Miles Driven: </b>"
            + x.astype(str)
            + "<br><b>Cost To Drive One Mile (in cents): </b>"
            + y.astype(str)
            + "<br>"
            + df["date"].dt.strftime("%b %-d, %Y")
            + "<extra></extra>"
        ...
        tick_text = [
            money_format(i)
            for i in [
                i / 100 for i in range(math.floor(y.min()), math.ceil(y.max()) + 1)
            ]
        ]
        annotation = (
            "On average, it cost me "
            + (round(df_i["Cost to go one mile (in cents)"].iloc[-1], 2)).astype(str)
    ...
    return {
        "data": [
            go.Scatter(x=x, y=y, mode="markers", hovertemplate=htemp, name="Data"),
            go.Scatter(
                x=x,
                y=preds,
                mode="lines",
                ...
                + "<br>r^2 = {}".format(r_2)
                + "<br>RMSE = {}".format(rmse),
                hovertemplate="<b>Miles Driven: </b>"
                + x.astype(str)
                + "<br><b>Predicted {}: </b>".format(graph_option)
                + round(preds, 2).astype(str)
                + "<extra></extra>",
            ),
        ],
        "layout": go.Layout(
            ...
            xaxis=dict(title="Miles Driven", range=[x.min() - 2, x.max() + 2]),
            yaxis=dict(
                title=graph_option, range=yrange, tickvals=ytick, ticktext=tick_text
            ),
            annotations=[
